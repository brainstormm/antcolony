...
<script>
		...
		$("#pass").blur(function(){
			...
			var pass= $("#pass").val();
			//window.alert(pass.length);
			if(pass.length<6 && pass!=""&&!isPass(pass))
			{
				$("#pass").css("border","2px solid red");
				$("#pass").val("");
				$("#pass").attr("placeholder","Password must consist of six or more characters");
				cpass = 0;
			}
		...
		function isPass(pass)
		{
			...
			//After using url encoding and decoding
			if (pass.length<6)
			{
				return false;
			}
...
function loginme()
{
	...
	var did = encodeURIComponent(document.getElementById("username").value);
	var dpwd = encodeURIComponent(document.getElementById("password").value);
