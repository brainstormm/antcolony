<?php
	...
	{
		$handle = oci_parse($connec,'Select key,confirmed,email from users where id = '."'".urldecode($_POST['username'])."'");
		oci_execute($handle);
		$ans = oci_fetch_array($handle,OCI_BOTH);
		if ($ans!=false&&$ans['CONFIRMED']==0&&pwds::verify_password(urldecode($_POST['password']),$ans[0]))
		{
			echo "ANV";
			verification($ans['EMAIL'],vhashfetch($ans['EMAIL']));
			exit();
		}
		elseif (pwds::verify_password(urldecode($_POST['password']),$ans[0]))
		{
			$_SESSION['login'] = 1;
			$_SESSION[urldecode($_POST['username'])] = 1;
			setcookie('antcolonylogin',urldecode($_POST['username']),time()+3600,'/',$_SERVER['HTTP_HOST'],true);
			echo 'Yes';
		}
		else
		{
