...
if (!isset($dat['DPNAME']))
			...
			<?php
				...
				if ($ret1 == false)
				{
					echo '<a href="messages.php" class="list-group-item active">Messages</a>';
				}
				else
				{
					echo '<a href="messages.php" class="list-group-item active"><span class="badge">'.$ret1['ICOUNT'].'</span>Messages</a>';
				}
			?>
		...
		<div class="container-fluid">
			<ul class="nav navbar-inverse navbar-right">
					<?php echo'<li ><a href="./sentmessages_user.php" style="background-color:white"><span class="glyphicon glyphicon-open"></span> Sent Mail</a></li>'; ?>
					</ul>
					<ul class="nav navbar-inverse navbar-right">
					<?php echo'<li><a href="./messages.php" style="background-color:white"><span class="glyphicon glyphicon-inbox"></span> Inbox</a></li>'; ?>
				</ul>
				</div>
			<br>
			...
			<!--<div class="col-md-11 col-md-offset-1" style="background-color: #ECF0F1; margin-top: 20px;">
				
				 <img src="gravatar_male.png" class="img-rounded"  width="100" height="100" style="float: left;">
				 <h6>Whatever message is received is displayed here....</h6><br><br>
				 <p>Sent by whoever, time</p>
				
			</div>-->
			<!--Unread message Part-->
<?php
	/*************************************************************/
	$handle = oci_parse($connec,"select count(sid) as ISCOUNT from unread where rid = '".$_COOKIE['antcolonylogin']."' group by rid");
	oci_execute($handle);
	$ret2 = oci_fetch_array($handle,OCI_BOTH);
	if ($ret2!=false)
	{
		$handle = oci_parse($connec,"select sid,rid,mes,upper(to_char(dos,'month DD, YYYY  hh:mi:ss')) as tocdos from unread where rid = '".$_COOKIE['antcolonylogin']."' order by dos desc");
		oci_execute($handle);
		$cntr_limit = $ret2['ISCOUNT'];
		for ($cntr = 0;$cntr<$cntr_limit;$cntr++)
		{
			$ret3 = oci_fetch_array($handle,OCI_BOTH);
			$ansu = dataid($connec,$ret3['SID']);
			echo '<div class="col-md-11 col-md-offset-1" style="background-color: #FFFFFF;  margin-top: 20px; border: solid 2px #adad85;">';
			echo '<p style="color:red"> <span style="color:black">Sent By </span> - '.ucwords($ansu['FNAME']).' '.ucwords($ansu['LNAME']).'</p><hr style="border-color:black">';
...
<?php
	/************************************************/
	$handle = oci_parse($connec,"select count(sid) as ISCOUNT from messages where rid = '".$_COOKIE['antcolonylogin']."' group by rid");
	oci_execute($handle);
	$ret4 = oci_fetch_array($handle,OCI_BOTH);
	if ($ret4!=false)
	{
		$handle = oci_parse($connec,"select sid,rid,mes,upper(to_char(dos,'month DD, YYYY  hh:mi:ss')) as tocdos from messages where rid = '".$_COOKIE['antcolonylogin']."' order by dos desc");
		oci_execute($handle);
		$cntr_limit = $ret4['ISCOUNT'];
		for ($cntr = 0;$cntr<$cntr_limit;$cntr++)
		{
			$ret5 = oci_fetch_array($handle,OCI_BOTH);
			$ansu = dataid($connec,$ret5['SID']);
			echo '<div class="col-md-11 col-md-offset-1" style="background-color: #FFFFFF;  margin-top: 20px; border: solid 2px #adad85;">';
			echo '<p style="color:red"> <span style="color:black">Sent By </span> - '.ucwords($ansu['FNAME']).' '.ucwords($ansu['LNAME']).'</p><hr style="border-color:black">';
			echo '<a href="'.$ansu['PURL'].'"><img src="'.$ansu['DP'].'" class="img-rounded"  width="100" height="100" style="float: left; margin:10px;"></a>';
	...
	$handle = oci_parse($connec,"insert into messages select * from unread where rid = '".$_COOKIE['antcolonylogin']."'");
	oci_execute($handle);
	$handle = oci_parse($connec,"delete from unread where rid = '".$_COOKIE['antcolonylogin']."'");
	oci_execute($handle);
?>
</div>
<script>
