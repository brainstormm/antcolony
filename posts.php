...
<a href="./globaluserlist.php" class="list-group-item">Global User List</a>
		...
		<div class="container-fluid">
			...
			<p style="color:green; margin-left:40px; visibility:hidden" id="sentok"><b>Post done</b></p>
			<?php echo '<button class="btn btn-success btn-lg" style="margin-left:43%" onclick="sendto(\''.$dat['ID'].'\')">Add Post</button>'; ?>
			<br>
			<hr style="border-color:black">
			<h3 style="color:red; text-align:center">Your Posts</h2>
			<hr style="border-color:black">
			<?php
	/*************************************************************/
	$handle = oci_parse($connec,"select count(pid) as ISCOUNT from posts where puid = '".$_COOKIE['antcolonylogin']."' group by puid");
	oci_execute($handle);
	$ret2 = oci_fetch_array($handle,OCI_BOTH);
	if ($ret2!=false)
	{
		$handle = oci_parse($connec,"select puid,pdata,pid,upper(to_char(dop,'month DD, YYYY  hh:mi:ss')) as tocdos from posts where puid = '".$_COOKIE['antcolonylogin']."' order by dop desc");
		oci_execute($handle);
...
?>
			 ...
			 <!--<div class="col-md-11 col-md-offset-1" style="background-color: #ECF0F1;  margin-top: 20px;">
				 ...
				 <img src="gravatar_male.png" class="img-circle"  width="80" height="80" style="float: left;">
				 <h5 style="margin-bottom: 0px;">Firstname lastname</h5>
				 <span style="font-size: 0.9em;">Time stamp</span>
				 <p style="margin-top: 9px; clear: left;">Baap kaa, bhaai ka sabkaa badla legaa re tera faizaaaa !</p>
