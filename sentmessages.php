...
if (!isset($dat['DPNAME']))
		...
		<div class="container-fluid">
			...
			<br>
			<?php echo '<h3 style="color:red; text-align:center;">Messages To '.$dat['FNAME'].' '.$dat['LNAME'].'</h3>'; ?>
			<hr style="border-color:black">
			<h6 style="color:red; text-align:center;">Unseen Messages</h6>
<!--Unread message Part-->
<?php
	/*************************************************************/
	$handle = oci_parse($connec,"select count(rid) as ISCOUNT from unread where sid = '".$_COOKIE['antcolonylogin']."' and rid = '".$dat['ID']."' group by sid");
	oci_execute($handle);
	$ret2 = oci_fetch_array($handle,OCI_BOTH);
	if ($ret2!=false)
	{
		$handle = oci_parse($connec,"select sid,rid,mes,upper(to_char(dos,'month DD, YYYY  hh:mi:ss')) as tocdos from unread where sid = '".$_COOKIE['antcolonylogin']."' and rid = '".$dat['ID']."' order by dos desc");
		oci_execute($handle);
...
<?php
	/************************************************/
	$handle = oci_parse($connec,"select count(rid) as ISCOUNT from messages where sid = '".$_COOKIE['antcolonylogin']."' and rid = '".$dat['ID']."' group by sid");
	oci_execute($handle);
	$ret4 = oci_fetch_array($handle,OCI_BOTH);
	if ($ret4!=false)
	{
		$handle = oci_parse($connec,"select sid,rid,mes,upper(to_char(dos,'month DD, YYYY  hh:mi:ss')) as tocdos from messages where sid = '".$_COOKIE['antcolonylogin']."' and rid = '".$dat['ID']."' order by dos desc");
		oci_execute($handle);
