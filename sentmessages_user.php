...
if (!isset($dat['DPNAME']))
		...
		<div class="container-fluid">
			...
			<br>
			<h6 style="color:red; text-align:center;">Unseen Messages</h6>
			<hr style="border-color:black">
			<!--Unread message Part-->
<?php
	/*************************************************************/
	$handle = oci_parse($connec,"select count(rid) as ISCOUNT from unread where sid = '".$_COOKIE['antcolonylogin']."' group by sid");
	oci_execute($handle);
	$ret2 = oci_fetch_array($handle,OCI_BOTH);
	if ($ret2!=false)
	{
		$handle = oci_parse($connec,"select sid,rid,mes,upper(to_char(dos,'month DD, YYYY  hh:mi:ss')) as tocdos from unread where sid = '".$_COOKIE['antcolonylogin']."' order by dos desc");
		oci_execute($handle);
		$cntr_limit = $ret2['ISCOUNT'];
		for ($cntr = 0;$cntr<$cntr_limit;$cntr++)
		{
			$ret3 = oci_fetch_array($handle,OCI_BOTH);
			$ansu = dataid($connec,$ret3['RID']);
			echo '<div class="col-md-11 col-md-offset-1" style="background-color: #FFFFFF;  margin-top: 20px; border: solid 2px #adad85;">';
			echo '<p style="color:red"> <span style="color:black">Sent To </span> - '.ucwords($ansu['FNAME']).' '.ucwords($ansu['LNAME']).'</p><hr style="border-color:black">';
...
<?php
	/************************************************/
	$handle = oci_parse($connec,"select count(rid) as ISCOUNT from messages where sid = '".$_COOKIE['antcolonylogin']."' group by sid");
	oci_execute($handle);
	$ret4 = oci_fetch_array($handle,OCI_BOTH);
	if ($ret4!=false)
	{
		$handle = oci_parse($connec,"select sid,rid,mes,upper(to_char(dos,'month DD, YYYY  hh:mi:ss')) as tocdos from messages where sid = '".$_COOKIE['antcolonylogin']."' order by dos desc");
		oci_execute($handle);
		$cntr_limit = $ret4['ISCOUNT'];
		for ($cntr = 0;$cntr<$cntr_limit;$cntr++)
		{
			$ret5 = oci_fetch_array($handle,OCI_BOTH);
			$ansu = dataid($connec,$ret5['RID']);
			echo '<div class="col-md-11 col-md-offset-1" style="background-color: #FFFFFF;  margin-top: 20px; border: solid 2px #adad85;">';
			echo '<p style="color:red"> <span style="color:black">Sent To </span> - '.ucwords($ansu['FNAME']).' '.ucwords($ansu['LNAME']).'</p><hr style="border-color:black">';
			echo '<a href="'.$ansu['PURL'].'"><img src="'.$ansu['DP'].'" class="img-rounded"  width="100" height="100" style="float: left; margin:10px;"></a>';
